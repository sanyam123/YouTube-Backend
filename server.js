...
async function fetchVideoMetadata(videoId) {
  ...
  // Instead of throwing an error, return a flag indicating if chapters were found
  return { 
    videoDetails, 
    chapters,
...
async function fetchTranscriptFromSearchAPI(videoId) {
  ...
  try {
    ...
    // If successful and has transcripts, return them
    if (response.data && response.data.transcripts && response.data.transcripts.length > 0) {
      console.log(`Found ${response.data.transcripts.length} transcript segments in English`);
    ...
    // If we get here without throwing an error but no transcripts, check available languages
    if (response.data.available_languages && response.data.available_languages.length > 0) {
      return await fallbackToAvailableLanguage(videoId, response.data.available_languages);
    }
    
    // No transcripts and no available languages
    throw new Error('No transcript found for this video in any language');
    
  } catch (error) {
    // If the original request failed and has available languages info
    if (error.response?.data?.available_languages && 
        error.response.data.available_languages.length > 0) {
      
      return await fallbackToAvailableLanguage(videoId, error.response.data.available_languages);
    }
    
    // For all other errors
    console.error('SearchAPI.io transcript fetch error:', error.message);
    throw new Error('No transcript found for this video');
  }
}

// Helper function to fetch transcript in the first available language
async function fallbackToAvailableLanguage(videoId, availableLanguages) {
  // Pick the first available language
  const firstLanguage = availableLanguages[0];
  console.log(`English transcript not available. Using ${firstLanguage.name} (${firstLanguage.lang}) instead`);
  
  try {
    // Make a new request with the first available language
    const fallbackResponse = await axios.get('https://www.searchapi.io/api/v1/search', {
      params: {
        engine: 'youtube_transcripts',
        video_id: videoId,
    ...
    if (fallbackResponse.data && fallbackResponse.data.transcripts && 
        fallbackResponse.data.transcripts.length > 0) {
      
      console.log(`Found ${fallbackResponse.data.transcripts.length} transcript segments in ${firstLanguage.name}`);
      
      // Process transcript data
      const transcriptData = fallbackResponse.data.transcripts.map(item => ({
        text: cleanHtmlEntities(item.text),
        offset: item.start,
        duration: item.duration
      }));
      ...
      const isAutoGenerated = firstLanguage.name.toLowerCase().includes('auto-generated');
      
      return { 
        transcriptData, 
        plainText,
        language: firstLanguage.lang,
        languageName: firstLanguage.name,
        isAutoGenerated,
        requiresTranslation: firstLanguage.lang !== 'en',
        allAvailableLanguages: availableLanguages
      };
    }
    
    // No transcripts found even in fallback language
    throw new Error('No transcript found for this video in any language');
    
  } catch (error) {
    console.error(`Error fetching ${firstLanguage.name} transcript:`, error.message);
    throw new Error(`Failed to get transcript in ${firstLanguage.name}`);
  }
}

// Endpoint to get video data and transcript
// Update your /api/video-data endpoint to handle transcript language
app.get('/api/video-data', async (req, res) => {
  try {
    ...
    try {
      ...
      if (metadataResult.hasChapters) {
        // Existing code path for videos with chapters
        ...
        // Include language information in the response
        const responseData = {
          ...
          transcriptLanguageName: transcriptResult.languageName || 'English',
          isAutoGenerated: transcriptResult.isAutoGenerated || false,
          requiresTranslation: transcriptResult.requiresTranslation || false,
          hasChapters: true
        };
        
        // If there are multiple available languages, include them in the response
        if (transcriptResult.allAvailableLanguages) {
          responseData.availableLanguages = transcriptResult.allAvailableLanguages;
        }
        
        console.log('Sending successful response for video with chapters');
        res.json(responseData);
      } else {
        // New code path for videos without chapters
        console.log('Video does not have chapters, returning raw transcript');
        
        // Return the same structure but with hasChapters: false
        const responseData = {
          videoDetails: metadataResult.videoDetails,
          transcript: transcriptResult.plainText,
          transcriptData: transcriptResult.transcriptData,
          organizedTranscript: [], // Empty array to maintain structure
          transcriptMethod: 'searchapi',
          transcriptLanguage: transcriptResult.language || 'en',
          transcriptLanguageName: transcriptResult.languageName || 'English',
          isAutoGenerated: transcriptResult.isAutoGenerated || false,
        ...
        console.log('Sending successful response for video without chapters');
        res.json(responseData);
      }
...
// IMPORTANT: All existing endpoints remain unchanged
...
// NEW ENDPOINT: Process non-chaptered video transcript in chunks
app.post('/api/process-transcript-chunk', async (req, res) => {
  try {
    const { transcriptChunk, previousContext, chunkIndex } = req.body;
    
    if (!transcriptChunk) {
      return res.status(400).json({ message: 'Invalid transcript chunk data' });
    }
    
    console.log(`Processing transcript chunk ${chunkIndex}`);
    
    // Process the chunk in parallel for both enhancement and analysis
    try {
      const [enhancedResult, analysisResult] = await Promise.all([
        // Enhance transcript
        (async () => {
          const enhancePrompt = `You are an expert transcript editor specializing in improving the readability of automatically generated YouTube video transcripts. Your task is to enhance this transcript segment while preserving its complete meaning and information.
...
${previousContext ? 'Context from previous section (for reference only):\n' + previousContext + '\n\n' : ''}
...
Provide the enhanced version that maintains all information but is easier to read.`;
          ...
          console.log('Calling OpenAI API to enhance transcript chunk');
          ...
          console.log('Transcript enhancement complete');
          return enhanceResponse.choices[0].message.content;
        })(),
        
        // Generate insights
        (async () => {
          const analysisPrompt = `You are an expert content analyst specializing in extracting valuable information from video transcripts. Analyze this transcript segment.

${previousContext ? 'Context from previous section:\n' + previousContext + '\n\n' : ''}
...
2. KEY TAKEAWAYS:
   ...
   - Use clear, concise language
   - Organize in bullet points
   - Connect to themes from previous sections where relevant
   - Do not include your own opinions or interpretation
   - Only include information explicitly mentioned in the transcript
   - Length should be appropriate to content
...
3. MEMORABLE QUOTES:
   ...
   - For each quote, include exact phrasing (do not paraphrase)
   - If there are no notable quotes, skip this section entirely
   - Limit to 1-2 quotes unless the section is particularly long
   - Prioritize quotes that relate to ongoing themes where applicable
...
${transcriptChunk}`;
          ...
          console.log('Calling OpenAI API for transcript analysis');
          ...
          return { summary, takeaways, quotes };
        })()
      ]);
      
      console.log('Both enhancement and analysis complete for chunk');
      
      // Return both results
      res.json({
        chunkIndex,
        enhancedTranscript: enhancedResult,
        analysis: analysisResult,
        // Return the last part of the transcript to use as context for the next chunk
        contextForNextChunk: transcriptChunk.split(' ').slice(-100).join(' ')
      });
      
    } catch (error) {
      console.error('Error processing transcript chunk:', error.message);
      if (error.response) {
        console.error('OpenAI API error details:', error.response.data);
      }
    ...
    console.error('Error in process-transcript-chunk:', error.message);
    console.error('Stack trace:', error.stack);
    
    res.status(500).json({ 
      message: 'Failed to process transcript chunk',
      error: error.message,
      stack: process.env.NODE_ENV === 'production' ? null : error.stack
    });
  }
